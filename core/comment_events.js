...
Blockly.Events.CommentCreate.prototype.toJson = function() {
  ...
  // Add properties needed for block.setCommentText and comment.setSize
  json['text'] = this.text;
  json['minimized'] = this.minimized;
  if (this.xy) {
    json['xy'] = Math.round(this.xy.x) + ',' + Math.round(this.xy.y);
  }
...
Blockly.Events.CommentCreate.prototype.fromJson = function(json) {
  ...
  if (json['xy']) {
    ...
    console.warn('CommentCreate event from JSON is missing "xy" property.');
    this.xy = new goog.math.Coordinate(0,0); // Default to prevent error, though position will be wrong.
  }
